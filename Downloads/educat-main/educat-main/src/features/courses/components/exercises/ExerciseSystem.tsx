...
export const ExerciseSystem = ({ classId }: ExerciseSystemProps) => {
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <Tabs defaultValue="ortografia" className="space-y-6">
        <TabsList className="grid w-full grid-cols-5">
          <TabsTrigger value="ortografia" className="flex items-center space-x-2">
            <PenTool className="h-4 w-4" />
            <span>Ortografia</span>
          </TabsTrigger>
          <TabsTrigger value="gramàtica" className="flex items-center space-x-2">
            <BookOpen className="h-4 w-4" />
            <span>Gramàtica</span>
          </TabsTrigger>
          <TabsTrigger value="dictats" className="flex items-center space-x-2">
            <Volume2 className="h-4 w-4" />
            <span>Dictats</span>
          </TabsTrigger>
          <TabsTrigger value="comprensió_lectora" className="flex items-center space-x-2">
            <BookOpen className="h-4 w-4" />
            <span>Lectura</span>
          </TabsTrigger>
          <TabsTrigger value="altres" className="flex items-center space-x-2">
              ...
              {getExercisesByType(type).length === 0 ? (
                <div className="col-span-full text-center py-12">
                  <div className="text-muted-foreground">
                    ...
                    <p className="text-lg mb-2">No hi ha exercicis disponibles</p>
                    <p className="text-sm">Torna més tard per trobar nous exercicis de {type}.</p>
                  </div>
                </div>
              ) : (
                getExercisesByType(type).map(exercise => (
                  <Card key={exercise.id} className="hover:shadow-card hover-lift transition-all">
                    ...
                    <CardContent>
                      <div className="space-y-3">
                        <p className="text-sm text-muted-foreground">
                          {exercise.questions.length} preguntes
                          {exercise.timeLimit && ` • ${exercise.timeLimit} min`}
                          {exercise.referenceText && ' • Amb text de referència'}
                          {exercise.textReference && ' • Font oficial'}
                        </p>
