...
export const OrthographyExercise = ({ exercise, onComplete, showFeedback = true, hideAnswersUntilComplete = false, allExercisesCompleted = false, category = 'ortografia' }: OrthographyExerciseProps) => {
  ...
  const shuffledOptions = useMemo(() => {
    if ((exercise.type === 'multiple_choice' || exercise.type === 'true_false') && exercise.options) {
      // Don't shuffle true/false
      if (exercise.type === 'true_false') return exercise.options;
  ...
  return (
    <Card className={`relative transition-all duration-200 ${submitted ? (isCorrect() ? 'border-green-200 bg-green-50' : 'border-red-200 bg-red-50') : 'hover:shadow-md'}`}>
      ...
      <CardContent className="space-y-4">
        ...
        {(exercise.type === 'multiple_choice' || exercise.type === 'true_false') && (() => {
          // Compute effective options: use shuffled options, or default to Cert/Fals for true_false
          const effectiveOptions = (shuffledOptions && shuffledOptions.length > 0)
            ? shuffledOptions
            ...
            >
              {effectiveOptions.map((option, index) => (
                <div
                  ...
                  className={`relative flex items-center space-x-3 p-3 rounded-lg border cursor-pointer transition-all ${submitted ? (
                ...
                >
                  {submitted && isCorrect() && option === exercise.correctAnswer && (
        ...
        <div className="flex items-center gap-2 pt-4 relative">
          {/* Main submit button particles - only for text/other types */}
          {(exercise.type !== 'multiple_choice' && exercise.type !== 'true_false') && (
            <SuccessParticles active={triggerParticles} />
          )}
          {!submitted ? (
            ...
            <div className="flex items-center gap-2">
              <Button
                ...
                className={isCorrect() ? "bg-green-600 hover:bg-green-700 text-white" : ""}
                disabled={isCorrect()}
              >
                {isCorrect() ? (
