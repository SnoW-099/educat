...
BEGIN
    ...
    IF EXISTS (
        ...
        DO UPDATE SET
            ...
            overall_score = (
                GREATEST(
                    CASE WHEN (SELECT type FROM public.exercises WHERE id = NEW.exercise_id) = 'grammar' THEN NEW.score ELSE student_progress.grammar_score END,
                    0
                ) +
                GREATEST(
                    CASE WHEN (SELECT type FROM public.exercises WHERE id = NEW.exercise_id) = 'vocabulary' THEN NEW.score ELSE student_progress.vocabulary_score END,
                    0
                ) +
                GREATEST(
                    CASE WHEN (SELECT type FROM public.exercises WHERE id = NEW.exercise_id) = 'listening' THEN NEW.score ELSE student_progress.listening_score END,
                    0
                ) +
                GREATEST(
                    CASE WHEN (SELECT type FROM public.exercises WHERE id = NEW.exercise_id) = 'speaking' THEN NEW.score ELSE student_progress.speaking_score END,
                    0
                ) +
                GREATEST(
                    CASE WHEN (SELECT type FROM public.exercises WHERE id = NEW.exercise_id) = 'reading' THEN NEW.score ELSE student_progress.reading_score END,
                    0
                ) +
                GREATEST(
                    CASE WHEN (SELECT type FROM public.exercises WHERE id = NEW.exercise_id) = 'writing' THEN NEW.score ELSE student_progress.writing_score END,
                    0
                )
            ) / 6.0,
            updated_at = NOW();
