...
-- Only allow system/triggers to modify progress data
CREATE POLICY "Only system can modify progress" 
ON public.student_progress 
FOR ALL 
USING (false)
WITH CHECK (false);
...
-- Only allow system/triggers to modify XP data
...
-- Security Fix 4: Restrict profile role updates
DROP POLICY IF EXISTS "Users can update their own profile" ON public.profiles;

-- New policy that prevents role changes
...
-- Only system can manage roles (no INSERT/UPDATE/DELETE for users)
